  ...
  return (
    <div className="bg-white rounded-xl shadow-md overflow-hidden p-6 flex-1 flex flex-col justify-between">
      <div className="flex gap-5">
        <div className="w-64 h-32 object-cover rounded-xl">
          <Image src="/placeholder.jpg" alt="placeholder" width={240} height={240} />
        </div>
        <div className="flex flex-col justify-between">
          <div>
...
const Residence = () => {
  const { id } = useParams();
  const parsedId = React.useMemo(() => parseInt(id as string), [id]);
  ...
  const {
    ...
    error: propertyError,
  } = useGetPropertyQuery(parsedId, { skip: isNaN(parsedId) });

  const userId = authUser?.cognitoInfo?.userId;
  const {
    data: leases,
    isLoading: leasesLoading,
  } = useGetLeasesQuery(parseInt(userId || '0'), {
    skip: !userId,
  });
